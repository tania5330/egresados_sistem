  ...
  const stats = [
    {
      ...
      value: data?.misPostulaciones ?? 0,
      description: "Total de ofertas aplicadas",
      icon: <Briefcase className="h-4 w-4" />,
      trend: data ? { value: data.postulacionesTrend, positive: data.postulacionesTrend >= 0 } : undefined,
    },
    {
      title: "Ofertas Vistas",
      value: data?.ofertasVistas ?? 0,
      description: "Ofertas visualizadas",
      icon: <Eye className="h-4 w-4" />,
      trend: data ? { value: data.vistasTrend, positive: data.vistasTrend >= 0 } : undefined,
    },
    {
      title: "Tasa de Respuesta",
      value: `${data?.tasaRespuesta ?? 0}%`,
      description: "Respuestas de empresas",
      icon: <MessageSquare className="h-4 w-4" />,
      trend: data ? { value: data.respuestaTrend, positive: data.respuestaTrend >= 0 } : undefined,
    },
  ];
